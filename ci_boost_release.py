...
class script(script_common):
    ...
    def command_build(self):
        ...
        for archive_file in archive_files:
            ...
            created_date = ""
            try:
                created_date = subprocess.check_output("set -e && set -o pipefail; TZ=UTC git -C " + self.root_dir + " show -s --date='format-local:%Y-%m-%dT%H:%M:%SZ' " + self.commit + " | grep Date: | tr -s ' ' | cut -d' ' -f 2 ", shell=True, executable='/bin/bash');
